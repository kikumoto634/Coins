...
void Area04::AppInitialize(WinApp *winapp, DirectXCommon *dxCommon, Input *input, TextureManager *textureManager, GeometryManager *geometryModel, Camera *camera, DebugText *debugText, SoundManager *soundManager)
{
	this->winApp = winapp;
...
void Area04::Finalize()
{
}

#pragma region コイン処理
void Area04::LoadCoinPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area04/CoinPopData.csv");
	assert(file.is_open());
...
void Area04::UpdateCoinPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area04::CoinPop(Vector3 pos)
{
...
#pragma region 壁処理
void Area04::LoadWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area04/WallPopData.csv");
	assert(file.is_open());
...
void Area04::UpdateWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area04::WallPop(Vector3 pos)
{
...
#pragma region スライド壁
void Area04::LoadSlideWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area04/SlideWallPopData.csv");
	assert(file.is_open());
...
void Area04::UpdateSlideWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area04::SlideWallPop(Vector3 pos)
{
	unique_ptr<Model> newModel = make_unique<Model>();
...
void Area04::CheckCollisionPair(Collider *colliderA, Collider *colliderB)
{
	//衝突フィルタリング
...
bool Area04::CheckCollisionDetail(Collider *colliderA, Collider *colliderB)
{
	//ボックス同士の当たり判定
...
void Area04::ObjectVelocityStop()
{
	//速度設定
