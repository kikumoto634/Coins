...
void GameBase::Run()
{
	//初期化
	GameBaseInitialize();
	AppInitialize(winApp.get(), dxCommon.get(), input.get(), textureManager.get(), geometryModel.get(), camera.get(), debugText.get(), soundManager.get());
	Initialize();
...
void GameBase::GameBaseInitialize()
{
