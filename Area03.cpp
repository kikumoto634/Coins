...
void Area03::AppInitialize(WinApp *winapp, DirectXCommon *dxCommon, Input *input, TextureManager *textureManager, GeometryManager *geometryModel, Camera *camera, DebugText *debugText, SoundManager *soundManager)
{
	this->winApp = winapp;
...
void Area03::Finalize()
{
}

#pragma region コイン処理
void Area03::LoadCoinPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area03/CoinPopData.csv");
	assert(file.is_open());
...
void Area03::UpdateCoinPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area03::CoinPop(Vector3 pos)
{
...
#pragma region 壁処理
void Area03::LoadWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area03/WallPopData.csv");
	assert(file.is_open());
...
void Area03::UpdateWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area03::WallPop(Vector3 pos)
{
...
#pragma region スライド壁
void Area03::LoadSlideWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area03/SlideWallPopData.csv");
	assert(file.is_open());
...
void Area03::UpdateSlideWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area03::SlideWallPop(Vector3 pos)
{
	unique_ptr<Model> newModel = make_unique<Model>();
...
void Area03::CheckCollisionPair(Collider *colliderA, Collider *colliderB)
{
	//衝突フィルタリング
...
bool Area03::CheckCollisionDetail(Collider *colliderA, Collider *colliderB)
{
	//ボックス同士の当たり判定
...
void Area03::ObjectVelocityStop()
{
	//速度設定
