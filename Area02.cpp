...
void Area02::AppInitialize(WinApp *winapp, DirectXCommon *dxCommon, Input *input, TextureManager *textureManager, GeometryManager *geometryModel, Camera *camera, DebugText *debugText, SoundManager *soundManager)
{
	this->winApp = winapp;
...
void Area02::Finalize()
{
}

#pragma region コイン処理
void Area02::LoadCoinPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area02/CoinPopData.csv");
	assert(file.is_open());
...
void Area02::UpdateCoinPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area02::CoinPop(Vector3 pos)
{
...
#pragma region 壁処理
void Area02::LoadWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area02/WallPopData.csv");
	assert(file.is_open());
...
void Area02::UpdateWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area02::WallPop(Vector3 pos)
{
...
#pragma region スライド壁
void Area02::LoadSlideWallPopData()
{
	//file開く
	ifstream file;
	file.open("Resources/csv/Area02/SlideWallPopData.csv");
	assert(file.is_open());
...
void Area02::UpdateSlideWallPopCommands()
{
	//一行分の文字列を入れる変数
	string line;
...
void Area02::SlideWallPop(Vector3 pos)
{
	unique_ptr<Model> newModel = make_unique<Model>();
...
void Area02::CheckCollisionPair(Collider *colliderA, Collider *colliderB)
{
	//衝突フィルタリング
...
bool Area02::CheckCollisionDetail(Collider *colliderA, Collider *colliderB)
{
	//ボックス同士の当たり判定
...
void Area02::ObjectVelocityStop()
{
	//速度設定
